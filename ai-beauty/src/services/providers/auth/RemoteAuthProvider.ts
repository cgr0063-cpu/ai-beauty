...
export class RemoteAuthProvider implements AuthProvider {
  ...
  async signInWithGoogle(payload: { email: string; name: string | null; googleSub: string }): Promise<AuthResult> {
    const { token, user } = await this.post("/v1/auth/google", payload);
    await this.persist(token, user);
    return { user, token, scope: "remote" };
  }

  async signInWithApple(payload: { email: string | null; name: string | null; appleSub: string }): Promise<AuthResult> {
